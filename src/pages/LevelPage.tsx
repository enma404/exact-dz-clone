...
const LevelPage = () => {
  ...
  // Check if this is a secondary level to show branches instead of subjects
  const { data: branches, isLoading: branchesLoading } = useQuery({
    ...
    enabled: level?.level_type === "secondary",
  });
  ...
  const isSecondary = level?.level_type === "secondary";
  const isLoading = levelLoading || (isSecondary ? branchesLoading : subjectsLoading);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        {/* Branches or Subjects Grid */}
        <section className="py-12">
          <div className="container mx-auto px-4">
            <h2 className="text-3xl font-bold text-white mb-8 text-center">
              {isSecondary ? "اختر شعبتك" : "المواد الدراسية"}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              {isSecondary ? (
                branches?.map((branch: any) => {
                  const Icon = getIcon(branch.icon_name);
                  const subjectsCount = branch.subjects?.[0]?.count || 0;
                  return (
                    <Card
                      key={branch.id}
